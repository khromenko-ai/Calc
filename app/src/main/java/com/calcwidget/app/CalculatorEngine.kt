...
object CalculatorEngine {
    ...
    private fun inputDecimal(ctx: Context) {
        if (isWaiting(ctx)) { save(ctx, "0.", "", "", false); return }
        val display = getDisplay(ctx)
        if (!display.contains('.')) save(ctx, "$display.", getPrev(ctx), getOperator(ctx), false)
    }

    private fun inputDigit(ctx: Context, digit: String) {
        val display = getDisplay(ctx)
        val new = if (isWaiting(ctx)) digit else if (display == "0") digit else display + digit
        save(ctx, new, getPrev(ctx), getOperator(ctx), false)
    }
    ...
    private fun performOp(ctx: Context, nextOp: String) {
        ...
        val newPrev  = if (prev.isEmpty()) display
                       else if (operator.isNotEmpty()) fmt(calculate(prev.toDoubleOrNull() ?: 0.0, input, operator))
                       else prev
        val newDisplay = if (prev.isNotEmpty() && operator.isNotEmpty())
            fmt(calculate(prev.toDoubleOrNull() ?: 0.0, input, operator)) else display
    ...
    private fun calcResult(ctx: Context) {
        ...
        val result = calculate(prev.toDoubleOrNull() ?: 0.0, display.toDoubleOrNull() ?: 0.0, operator)
    ...
    private fun calculate(a: Double, b: Double, op: String): Double {
        ...
        return if (r.isNaN() || r.isInfinite()) r else "%.12g".format(r).toDouble()
    }

    fun fmt(d: Double): String {
        if (d.isNaN()) return "Error"
        if (d.isInfinite()) return if (d > 0) "∞" else "-∞"
        return "%.10g".format(d).let { if (it.contains('.')) it.trimEnd('0').trimEnd('.') else it }
    }

    private fun save(ctx: Context, display: String, prev: String, op: String, waiting: Boolean) {
        prefs(ctx).edit()
